    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-lg mb-6">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <!-- Min Amount -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Min Amount ({{ $account->currency }})</label>
                        <input 
                            type="number" 
                    ...
                    <!-- Max Amount -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Max Amount ({{ $account->currency }})</label>
                        <input 
                            type="number" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow-lg p-4">
                ...
                <p class="text-xl font-bold text-gray-900">{{ $account->currency }} {{ number_format($openingBalance, 2) }}</p>
            ...
            <div class="bg-white rounded-lg shadow-lg p-4">
                ...
                <p class="text-xl font-bold text-green-600">{{ $account->currency }} {{ number_format($totalCredits, 2) }}</p>
                <p class="text-xs text-gray-500">{{ $transactionCount }} transactions</p>
            </div>

            <div class="bg-white rounded-lg shadow-lg p-4">
                <p class="text-sm font-medium text-gray-500">Total Debits</p>
                <p class="text-xl font-bold text-red-600">{{ $account->currency }} {{ number_format($totalDebits, 2) }}</p>
                <p class="text-xs text-gray-500">{{ $transactionCount }} transactions</p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-lg p-4">
                ...
                <p class="text-xl font-bold text-gray-900">{{ $account->currency }} {{ number_format($closingBalance, 2) }}</p>
                        ...
                        @forelse($transactions as $transaction)
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs font-medium rounded-full
                                        @if($transaction->type == 'deposit' || $transaction->type == 'cash_deposit') bg-green-100 text-green-800
                                        @elseif($transaction->type == 'withdrawal' || $transaction->type == 'cash_withdrawal') bg-red-100 text-red-800
                                        @elseif($transaction->type == 'transfer') bg-blue-100 text-blue-800
                                        @elseif($transaction->type == 'fee') bg-yellow-100 text-yellow-800
                                        @elseif($transaction->type == 'interest') bg-purple-100 text-purple-800
                                        @else bg-gray-100 text-gray-800 @endif">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-gray-900">
                                    {{ $account->currency }} {{ number_format($runningBalance, 2) }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs font-medium rounded-full
                                        ...
                                        @elseif($transaction->status === 'reversed') bg-gray-100 text-gray-800
                                        @else bg-gray-100 text-gray-800 @endif">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <div class="space-y-3">
                                            <label class="flex items-center">
                                                <input type="radio" wire:model="exportFormat" value="pdf" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                                                <span class="ml-3">
                                                    <span class="block text-sm font-medium text-gray-700">PDF Document</span>
                                                    <span class="block text-xs text-gray-500">Best for printing and sharing</span>
                                                </span>
                                            </label>
                                            
                                            <label class="flex items-center">
                                                <input type="radio" wire:model="exportFormat" value="csv" class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                                                <span class="ml-3">
                                                    <span class="block text-sm font-medium text-gray-700">CSV File</span>
                                                    <span class="block text-xs text-gray-500">Compatible with Excel and other spreadsheet apps</span>
                                                </span>
                                            </label>
                                            
                                            <label class="flex items-center opacity-50">
                                                <input type="radio" wire:model="exportFormat" value="excel" disabled class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300">
                                                <span class="ml-3">
                                                    <span class="block text-sm font-medium text-gray-700">Excel File (Coming Soon)</span>
                                                    <span class="block text-xs text-gray-500">Native Excel format</span>
                                                </span>
                                            </label>
                                        </div>
                                    </div>
